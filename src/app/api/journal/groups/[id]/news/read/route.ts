...
import { markJournalNewsItemRead } from "@/lib/journal-news";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try { const { id } = await context.params; const marked = await markJournalNewsItemRead(await getDb(), id, await request.json(), "journalTradeGroups"); return marked ? NextResponse.json({ ok: true }) : NextResponse.json({ error: "Journal group not found." }, { status: 404 }); }
